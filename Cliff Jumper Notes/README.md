<p align="center"><img width="651" height="968" alt="ChatGPT Image Jun 11, 2026, 12_49_40 PM" src="https://github.com/user-attachments/assets/99da5453-5993-4993-8aac-51c0a4b6510c" /></p>
...
There is one note per module (Modules 1 through 14; Module 7 is split into
...
| 14 | Very High (~97%) | Every statsmodels/patsy/scikit-learn call and number run on statsmodels 0.14.5 / patsy 1.0.1 / scikit-learn 1.7.2 / pandas 2.3.3 across the module's four datasets (the 97-row food-truck data, the 150-apartment synthetic CSV, a seed-2100 noisy sine, and seaborn's penguins): the food-truck fit reproduced to full precision (R-squared 0.7020315537841397, residuals summing to zero at 3.95e-14), the squared-correlation reading of R-squared pinned to its true scope (OLS with an intercept scored on its own training data, with executed counterexamples where the general 1 - SSres/SStot goes negative: -79.0 on a 4-row invented set and -2.758 on a deliberately backwards food-truck predictor), RMSE 35,290.12 tied to R-squared through RMSE = sd x sqrt(1 - R2), the five-model training table exposing the in-sample trap (a degree-8 polynomial out-scores the true sine model on training data), the full seven-model cross-validation table reproduced digit for digit under the module's constant KFold(n_splits=5, shuffle=True, random_state=9) with the one-standard-error threshold 35,464.73 + 2,278.50 = 37,743.22 selecting the simpler additive model over the genuinely significant interaction (the choice held in 19 of 20 fresh seeds, the interaction's raw win in 20 of 20), and the penguins pipeline mean RMSE 6.5350098073 reproduced three ways (pipeline, hand-rolled fold loop, and no scaler at all, identical because OLS is unmoved by linear rescaling of inputs, with KNN and Ridge contrasts verified to show where scaling does matter); full Diamond QC where the fidelity auditor caught the one wrong dataset pointer on the overfitting exhibit, the execution auditor's sole numeric challenge was overturned by the meta-auditor re-running the source note's own construction, and the meta-auditor's five minimal fixes were applied and re-verified |
